...
class PersonaCreator:
    ...
    def generate_complete_persona(self, traits, text_samples=None, quirk=None):
        """Generate a complete persona profile using LLM"""
        if self.demo_mode:
            return self._generate_fallback_persona(traits, quirk)
        ...
        if quirk:
            quirk_text = f"\nTheir unique behavioral or linguistic quirk is: {quirk}.\n"
        else:
            quirk_text = ""
...
Respond in JSON format with fields for each aspect of the persona."""
    ...
    def _generate_fallback_persona(self, traits, quirk=None):
        ...
        return {
            ...
            "quirk": quirk if quirk else "has a calm, adaptive tone with occasional humor"
        }
